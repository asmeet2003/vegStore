...
// const CartProduct = ({ id, name, image, category, qty, total, price }) => {
//   const dispatch = useDispatch();
//   const [quantity, setQuantity] = useState(qty); // Initialize quantity state with the prop value
...
//   const handleDecreaseQty = () => {
//     if (quantity > 1) {
//       setQuantity(quantity - 1); // Decrement the quantity state only if it's greater than 1
//       dispatch(decreaseQty(id)); // Dispatch action to decrease quantity in Redux store
//     }
//   };
...
const CartProduct = ({ id, name, image, category, qty, total, price }) => {
  ...
  const calculateTotalPrice = () => {
    return quantity * price; // Calculate total price based on current quantity
  };
